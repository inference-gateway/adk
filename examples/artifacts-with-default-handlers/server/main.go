...
func main() {
	...
	cfg.A2A.ArtifactsConfig = serverConfig.ArtifactsConfig{
		...
		ServerConfig: serverConfig.ArtifactsServerConfig{
			Host: "localhost",
			Port: "8081", // in production you might want to not hardcode it for better flexibility
		},
	...
	// Create toolbox with artifact-creating tools
...
`, filename, len(content), time.Now().Format(time.RFC3339), content)
			...
			// Store the analysis report using the artifacts server
			if artifactsServer != nil {
				storage := artifactsServer.GetStorage()
				artifactID := fmt.Sprintf("analysis-%s", time.Now().Format("20060102-150405"))
				reportFilename := fmt.Sprintf("analysis_%s.md", filename)

				url, err := storage.Store(ctx, artifactID, reportFilename, strings.NewReader(analysisReport))
				if err == nil {
			...
			// Create artifact using ArtifactHelper for the default handler to extract
			...
			artifact := artifactHelper.CreateFileArtifactFromBytes(
				...
				fmt.Sprintf("analysis_%s.md", filename),
				[]byte(analysisReport),
				&mimeType,
			)

			return fmt.Sprintf(`File processed successfully!

**Uploaded file**: %s
**Content length**: %d bytes

I've analyzed the uploaded file and created a comprehensive analysis report as a downloadable artifact.

The report includes:
- File metadata and processing details
- Complete content analysis
- Key insights about the upload process
- Demonstration of bidirectional artifact exchange
...
}`, filename, len(content), artifact.ArtifactID, *artifact.Name, *artifact.Description, filename, filename, artifact.Parts[0].(types.FilePart).File.(types.FileWithBytes).Bytes), nil
		},
	)
...
`, topic, topic, topic, topic, topic, topic, topic, time.Now().Format(time.RFC3339), "auto-generated")
			...
			// Store the report using the artifacts server if available
			if artifactsServer != nil {
				storage := artifactsServer.GetStorage()
				artifactID := fmt.Sprintf("report-%s-%s", topic, time.Now().Format("20060102-150405"))

				url, err := storage.Store(ctx, artifactID, filename, strings.NewReader(content))
				if err == nil {
					logger.Info("artifact stored in filesystem",
						zap.String("artifact_id", artifactID),
			...
			// Create artifact using ArtifactHelper
			artifactHelper := server.NewArtifactHelper()
			artifact := artifactHelper.CreateFileArtifactFromBytes(
			...
			// Return tool result with embedded artifact
			// The default task handler will automatically extract this artifact
...
The report has been created as a downloadable artifact. The default task handler will automatically extract and attach it to the task response.
...
}`, topic, format, filename, artifact.ArtifactID, *artifact.Name, *artifact.Description, filename, filename, mimeType, artifact.Parts[0].(types.FilePart).File.(types.FileWithBytes).Bytes), nil
		},
	)
...
@enduml`, title, description)
			...
			// Return tool result with embedded artifact
			return fmt.Sprintf(`Diagram created successfully!
...
The PlantUML diagram has been created as a downloadable artifact. You can use this with any PlantUML renderer to generate the visual diagram.
...
}`, title, diagramType, description, filename, artifact.ArtifactID, *artifact.Name, *artifact.Description, filename, filename, mimeType, artifact.Parts[0].(types.FilePart).File.(types.FileWithBytes).Bytes), nil
		},
	)
...
3,"Sample Item 3",300,"%s","%s"`, dataset, time.Now().Format("2006-01-02"), dataset, time.Now().Format("2006-01-02"), dataset, time.Now().Format("2006-01-02"))
			...
			// Return tool result with embedded artifact
			return fmt.Sprintf(`Data export completed successfully!
...
The data has been exported and saved as a downloadable artifact. The default task handler will automatically extract and attach it to the task response.
...
}`, dataset, format, filename, artifact.ArtifactID, *artifact.Name, *artifact.Description, filename, filename, mimeType, artifact.Parts[0].(types.FilePart).File.(types.FileWithBytes).Bytes), nil
		},
	)
