...
func (h *ArtifactsTaskHandler) HandleTask(ctx context.Context, task *types.Task, message *types.Message) (*types.Task, error) {
	...
	h.logger.Info("processing task with artifacts", zap.String("input", userInput))

	// Create sample content for the artifact
	artifactContent := fmt.Sprintf(`# Analysis Report

User Request: %s

## Summary
This is a sample analysis report generated by the artifacts filesystem agent.
...
## Conclusions
This demonstrates how artifacts can be created and made available for download
through the filesystem storage provider.
`, userInput, time.Now().Format(time.RFC3339), task.ID)
	...
	responseText := fmt.Sprintf("I've created an analysis report for your request: \"%s\". The report has been saved as an artifact and is available for download.", userInput)
