...
// RedisStorage is another example showing how to implement Redis-backed storage
type RedisStorage struct {
	logger *zap.Logger
	// redisClient redis.Client // Redis client would go here
}

// NewRedisStorage creates a new Redis-backed storage instance
func NewRedisStorage(logger *zap.Logger, redisURL string) (*RedisStorage, error) {
	if logger == nil {
		logger = zap.NewNop()
	}
...
// Implement Storage interface methods for Redis...
// (Similar pattern to DatabaseStorage but using Redis commands)

func (r *RedisStorage) StoreTask(task *types.Task) error {
	// Example Redis commands:
	// HSET task:{taskID} id {taskID} context_id {contextID} status {status} created_at {timestamp}
	// SADD context:{contextID}:tasks {taskID}
	// ZADD tasks:by_state {timestamp} {taskID}
	return fmt.Errorf("redis storage not implemented - this is just an example")
}

func (r *RedisStorage) GetTask(taskID string) (*types.Task, bool) {
	// Example Redis command: HGETALL task:{taskID}
	return nil, false
}

func (r *RedisStorage) GetTaskByContextAndID(contextID, taskID string) (*types.Task, bool) {
	// Example Redis commands:
	// SISMEMBER context:{contextID}:tasks {taskID}
	// HGETALL task:{taskID}
	return nil, false
}

func (r *RedisStorage) UpdateTask(task *types.Task) error {
	return fmt.Errorf("redis storage not implemented - this is just an example")
}

func (r *RedisStorage) DeleteTask(taskID string) error {
	return fmt.Errorf("redis storage not implemented - this is just an example")
}

func (r *RedisStorage) ListTasks(filter TaskFilter) ([]*types.Task, error) {
	return nil, fmt.Errorf("redis storage not implemented - this is just an example")
}

func (r *RedisStorage) ListTasksByContext(contextID string, filter TaskFilter) ([]*types.Task, error) {
	return nil, fmt.Errorf("redis storage not implemented - this is just an example")
}

func (r *RedisStorage) GetConversationHistory(contextID string) []types.Message {
	return []types.Message{}
}

func (r *RedisStorage) UpdateConversationHistory(contextID string, messages []types.Message) {
	// Implementation would go here
}

func (r *RedisStorage) AddMessageToConversation(contextID string, message types.Message) error {
	return fmt.Errorf("redis storage not implemented - this is just an example")
}

func (r *RedisStorage) TrimConversationHistory(contextID string, maxMessages int) error {
	return fmt.Errorf("redis storage not implemented - this is just an example")
}

func (r *RedisStorage) GetContexts() []string {
	return []string{}
}

func (r *RedisStorage) GetContextsWithTasks() []string {
	return []string{}
}

func (r *RedisStorage) DeleteContext(contextID string) error {
	return fmt.Errorf("redis storage not implemented - this is just an example")
}

func (r *RedisStorage) DeleteContextAndTasks(contextID string) error {
	return fmt.Errorf("redis storage not implemented - this is just an example")
}

func (r *RedisStorage) CleanupCompletedTasks() int {
	return 0
}

func (r *RedisStorage) CleanupOldConversations(maxAge int64) int {
	return 0
}

func (r *RedisStorage) GetStats() StorageStats {
	return StorageStats{
		TotalTasks:                0,
		TasksByState:              make(map[string]int),
		TotalContexts:             0,
