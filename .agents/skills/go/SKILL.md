...
> **In this repo:** the two public surfaces are `server/` (build an A2A agent)
> and `client/` (call one), with `types/` for the shared A2A wire types and
> `server/config/` for configuration. That is the established structure - follow
> it; don't invent a `service`/`repository` layer split. spf13's transferable
> rules still hold: name by purpose, no junk-drawer packages, wire at the
> composition root (here, the `A2AServerBuilder` / `AgentBuilder`).
...
> **In this repo:** the deliberate interface seams are `TaskHandler` /
> `StreamableTaskHandler`, `LLMClient`, `Storage`, and the artifact storage
> interfaces - these exist precisely because they get swapped (custom handlers,
> in-memory vs Redis storage, filesystem vs MinIO artifacts) and mocked. Add a
> new interface only when it earns a real second implementation or a generated
> mock; keep it small.
...
> **In this repo:** construction goes through the fluent builders
> (`A2AServerBuilder`, `AgentBuilder`, `ArtifactsServerBuilder`) with
> `WithXxx(...)` methods - the same "sane defaults, override as needed"
> philosophy. Extend a builder with a new `WithXxx` rather than adding a
